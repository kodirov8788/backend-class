...
userSchema.statics.signup = async function (username, password, name, lastname, number) {

  if (!username || !password, !number || !name || !lastname) {
  ...
  const user = await this.create({ username, password: hash, number, name, lastname })
  return user
}
